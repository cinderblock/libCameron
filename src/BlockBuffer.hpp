...
public:
   ...
   * @return
   */
  inline bool isReadableNow() const { return currentRead < Blocks; }

  /**
   * Check if there is room to write another block to the buffer
   * @return
   */
  inline bool isWriteableNow() const { return currentWrite < Blocks; }

  /**
   * Get the buffer that should be written to
   * @return
   */
  u1 *getWriteBuffer() { return isWriteableNow() ? buffers[currentWrite] : nullptr; }

  /**
   * Get the buffer that should be read from
   * @return
   */
  u1 *getReadBuffer() { return isReadableNow() ? buffers[currentRead] : nullptr; }

  u1 constexpr getBlocks() { return Blocks; }
