...
void BlockBuffer<BlockSize, Blocks, readInterrupt, writeInterrupt>::markCurrentWriteBufferAsDone() {
  ...
  if (nextWrite >= Blocks) nextWrite = 0;

  if (currentRead == 0xff) currentRead = curr;

  if (nextWrite == currentRead) nextWrite = 0xff;
...
void BlockBuffer<BlockSize, Blocks, readInterrupt, writeInterrupt>::markCurrentReadBufferAsDone() {
  ...
  if (nextRead >= Blocks) nextRead = 0;

  if (currentWrite == 0xff) currentWrite = curr;

  if (nextRead == currentWrite) nextRead = 0xff;
